...
static void doTreeSearchBetween(Tree t, Node n, Record lower,
    ...
    /*
    if left is in range, go left
    if right is not in range(discard the right of right), go to left of right and check again
    */

    if (n->left != NULL) {
        if ((t->compare(lower, n->left->rec)) > 0 && 
            (t->compare(upper, n->right->rec)) < 0) {
            ListAppend(l, n->rec);
        }
    }
    
    // comparing lower : 2nd > 1st
    int cmp_lower = t->compare(lower, n->rec);

    // comparing higher : 2nd < 1st
    int cmp_higher = t->compare(upper, n->rec);

    if (cmp_lower <= 0 && cmp_higher >= 0) {
