...
bool FbUnfriend(Fb fb, char *name1, char *name2) {
    bool isFriend = FbIsFriend(fb, name1, name2);
    if (isFriend != false) {
        int id1 = nameToId(fb, name1);
        int id2 = nameToId(fb, name2);
        assert(id1 != id2);
...
List FbMutualFriends(Fb fb, char *name1, char *name2) {
    ...
    while (i < fb->numPeople) {
        if (FbIsFriend(fb, name1, fb->names[i])) {
            if (FbIsFriend(fb, name2, fb->names[i])) {
...
void FbFriendRecs1(Fb fb, char *name) {
    ...
    int highest_m = 0;
    while (i < fb->numPeople) {
        List temp = FbMutualFriends(fb, name, fb->names[i]);
        bool isfriend = FbIsFriend(fb, name, fb->names[i]);
        int num_mutual = ListSize(temp);
        int compare = strcmp(name, fb->names[i]);
        if (num_mutual > 0 && isfriend == false && compare != 0) {
            if (num_mutual > highest_m) {
                highest_m = num_mutual;
            }
        }
        i++;
        ListFree(temp);
    ...
    while (i < fb->numPeople) {
        j = 0;
        while (j < fb->numPeople) {
            List temp = FbMutualFriends(fb, name, fb->names[j]);
            bool isfriend = FbIsFriend(fb, name, fb->names[j]);
            int num_mutual = ListSize(temp);
            int compare = strcmp(name, fb->names[j]);
            if (num_mutual == highest_m && isfriend == false && compare != 0) {
                printf("\t%-20s%4d mutual friends\n", fb->names[j], num_mutual);
            }
            j++;
            ListFree(temp);
        }
        highest_m = highest_m - 1;
        i++;
    } 
}
